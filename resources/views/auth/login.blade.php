<x-guest-layout>
    ...
    <div class="min-w-screen min-h-screen flex items-center justify-center p-3">
        <div class="text-textotitulo rounded-3xl shadow-md shadow-bgshadowicono w-full overflow-hidden max-w-lg">
            <div class="md:flex w-full">
                <div class="hidden md:flex items-center justify-center w-72 bg-bgnavlink p-5">
                    <svg id="a87032b8-5b37-4b7e-a4d9-4dbfbe394641" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg"
                        class="w-full h-auto" viewBox="0 0 744.84799 747.07702">
                ...
                <div class="w-full p-5">
                    <div class="text-center mb-5">
                        <h1 class="font-bold text-3xl text-textoprincipal">LOGIN</h1>
                        <p class="text-textoprincipal">{{ config('app.name', 'Sistema Web') }}</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('login') }}">
                        ...
                        <div class="w-full py-1">
                            ...
                            <x-input-text id="email" class="block w-full" type="email" name="email"
                                :value="old('email')" required autofocus />
                        </div>

                        <div class="w-full py-1">
                            <x-jet-label for="password" value="{{ __('Password') }}" />
                            <x-input-text id="password" class="block w-full" type="password" name="password"
                                required autocomplete="current-password" />
                        </div>

                        <div class="w-full flex flex-wrap items-center gap-3 mt-4 justify-between">
                            <label for="remember_me" class="flex items-center cursor-pointer">
                                <x-jet-checkbox id="remember_me" name="remember" />
                                <span class="ml-2 text-xs text-textoprincipal">{{ __('Remember me') }}</span>
                            </label>
                            @if (Route::has('password.request'))
                                <a class="underline text-xs text-textoprincipal hover:text-hoverbotondefault"
                        ...
                        <div class="flex items-center justify-center mt-4">
                            <x-button-default class="p-3 px-5" type="submit"> {{ __('Log in') }}
                            </x-button-default>
                            {{-- <button
                                class="inline-block mx-auto bg-indigo-500 hover:bg-indigo-700 focus:bg-indigo-700 text-white rounded-lg px-5 py-3 font-semibold">
                                {{ __('Log in') }}
                            </button> --}}
                        </div>
                    </form>
